 ...
 * @date 2019/4/19
 */
public class BiddOrderThread implements Runnable {
    ...
    public void run() {
        while (true) {
            log.info("Check BIDD product is expire, timeMillis is " + System.currentTimeMillis());
            try {
                Thread.sleep(1000);
    ...
    private void processExpireProd(ProductDo productDo) {
        ...
        if (biddResult == null) {
            //无竞价记录，流拍处理
            productDao.takeUpOrDownProd(productDo.getProductId());
        } else {
            //取最高价生成订单
