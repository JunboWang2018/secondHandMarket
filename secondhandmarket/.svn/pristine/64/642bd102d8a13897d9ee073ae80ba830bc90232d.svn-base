...
2. 将发布包中的 `/config/second-hand-market/` 目录放入 `/icooper/config/` 目录,并进行相应的配置（如果有必要）。
3. 将发布包中的 `second-hand-market.war` 放入 `{tomcat}/webapps/` 目录。

#### 升级安装
1. 执行发布包中的 `/sql/update{xxx}.sql` SQL脚本，执行数据库升级。
2. （如果有必要）修改 `/icooper/config/second-hand-market/` 目录中的配置文件。
3. 将发布包中的 `second-hand-market.war` 覆盖到 `{tomcat}/webapps/` 目录中。
...
如：`http://192.168.101.9:8080/second-hand-market/view/loginTo?view=/view/home&token=abcdef123456`。
...
### 示例
second-hand-market_r1.4.0.32_20170315_police.tgz
  ├─ README.md
  ├─ CHANGELOG.md
  ├─ second-hand-market.war
  ├─ sql/
...
- second-hand-market-rest.war - 发布版本的WAR包。
...
- config/ - 配置文件模板存放目录；由安装程序将该目录下的文件复制到/icooper/config/second-hand-market目录下，并替换模板文件内对应的配置项为正确的配置值。
...
用户配置的文件模板在： `{tgz包}/config/second-hand-market`目录中。
...
（这里的 {app-name} 默认为 `second-hand-market`）

如：`C:\Users\Administrator\scooper\second-hand-market`
...
如：`/icooper/config/second-hand-market`
...
运行完成后，程序打包并输出到共享目录：
```
\\192.168.103.154\132rd\Projects\second-hand-market\publish
```
